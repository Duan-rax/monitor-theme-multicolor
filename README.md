...
CPU、内存、硬盘与流量配额共用四档健康阈值：健康 `<60%`、负载 `60–74%`、
严重 `75–89%`、危险 `≥90%`，分别使用绿、蓝、橙、红显示。首页会按 CPU、单核归一化负载、内存、硬盘和流量中的最高等级，为整张节点卡片着色并显示状态标签；在线时长满一天后只显示天数。
网络延迟页面的各探测曲线统一使用实线，通过颜色和图例区分；丢包率独立显示在图例中。
